    ...
    <div className="relative min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-950 flex items-center justify-center px-6 py-12 transition-colors duration-300">
      ...
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}
        ...
        <form className="grid gap-4" onSubmit={handleEmailSignup}>
          ...
          {/* Alternative option */}
          <div className="mt-4 text-center text-sm text-slate-500 dark:text-slate-400">or sign up with Google</div>
          <button 
            type="button"
          ...
          <div className="mt-4 text-center text-sm text-slate-600 dark:text-slate-400">
            Already have an account? <button type="button" onClick={() => navigate('/login')} className="text-blue-600 dark:text-blue-400 hover:underline transition-opacity duration-200 hover:opacity-80">Login</button>
          </div>
        </form>
